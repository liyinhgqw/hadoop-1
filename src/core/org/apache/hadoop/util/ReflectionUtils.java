...
import org.apache.hadoop.io.Copyable;
...
public class ReflectionUtils {
	...
	static boolean foundConf;
	static Class<?> jobConfClass;
	static Class<?> jobConfigurableClass;
	static Method configureMethod;
	...
	private static void setJobConf(Object theObject, Configuration conf) {
		...
		if (!foundConf) {
			try {
				jobConfClass = conf
						.getClassByName("org.apache.hadoop.mapred.JobConf");
				jobConfigurableClass = conf
						.getClassByName("org.apache.hadoop.mapred.JobConfigurable");
				configureMethod = jobConfigurableClass.getMethod("configure",
						jobConfClass);
			} catch (ClassNotFoundException e) {
				// JobConf/JobConfigurable not in classpath. no need to
				// configure
			} catch (Exception e) {
		...
		if (jobConfClass == null || jobConfigurableClass == null) {
			return;
		}

		try {
			if (jobConfClass.isAssignableFrom(conf.getClass())
					&& jobConfigurableClass.isAssignableFrom(theObject
							.getClass())) {
	...
	public static void cloneWritableInto(Writable dst, Writable src)
		...
		try{
		  if ( (dst instanceof Copyable) 
				&& (src instanceof Copyable) ) {
			Copyable srcCopy = (Copyable) src;
			Copyable dstCopy = (Copyable) dst;
			srcCopy.copyField(dstCopy);
			return;
		  } 
		} catch (IOException e) {	
		}
